...
static inline bleResult_t ScDbGPS_UpdateGPSCharacteristicUTF8s(uint16_t *handle,
		char *data);

static bleResult_t ScDbGPS_RecordCharacteristicFloatToString(
		uint16_t serviceHandle, bleUuid_t* uuid_char, float* floatVal);
...
bleResult_t ScDbGPS_RecordGPSPosition(uint16_t serviceHandle, float* latitude, float* longitude) {
    ...
    result = GattDb_FindCharValueHandleInService(serviceHandle,
        gBleUuidType128_c, (bleUuid_t*)uuid_gps_position, &handle);
...
bleResult_t ScDbGPS_RecordGPSSpeed(uint16_t serviceHandle, float* speed) {
	return ScDbGPS_RecordCharacteristicFloatToString(serviceHandle,
			(bleUuid_t*) uuid_gps_speed, speed);
}

bleResult_t ScDbGPS_RecordGPSCourse(uint16_t serviceHandle, float* course) {
	return ScDbGPS_RecordCharacteristicFloatToString(serviceHandle,
			(bleUuid_t*) uuid_gps_course, course);
}
...
bleResult_t ScDbGPS_RecordGPSTime(uint16_t serviceHandle, struct minmea_time *time) {
	...
	/* Get handle of GPS latitude characteristic */
	result = GattDb_FindCharValueHandleInService(serviceHandle,
		gBleUuidType128_c, (bleUuid_t*)uuid_gps_time, &handle);

	if (result != gBleSuccess_c)
		return result;

	snprintf(strFloat, sizeof(strFloat), "%d:%d:%d",
...
bleResult_t ScDbGPS_RecordGPSDate(uint16_t serviceHandle, struct minmea_date *date) {
	...
	/* Get handle of GPS latitude characteristic */
	result = GattDb_FindCharValueHandleInService(serviceHandle,
		gBleUuidType128_c, (bleUuid_t*)uuid_gps_date, &handle);
...
static bleResult_t ScDbGPS_RecordCharacteristicFloatToString(uint16_t serviceHandle,
		bleUuid_t* uuid_char, float *floatVal) {
	uint16_t handle;
	bleResult_t result;
	char strFloat[16];
	...
	/* Get handle of GPS latitude characteristic */
	result = GattDb_FindCharValueHandleInService(serviceHandle,
			gBleUuidType128_c, (bleUuid_t*) uuid_char, &handle);
...
static void ScDbGPS_SendNotifications(uint16_t *charHandle) {
	...
	if (GattDb_FindCccdHandleForCharValueHandle(*charHandle, &cccdHandle)
			!= gBleSuccess_c)
		return;
